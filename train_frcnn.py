...
def main():
    ...
    for epoch in range(init_epochs):
        ...
        writer.add_scalar('trainloss', mean_loss, epoch + 1)
        writer.add_scalar('lr', lr, epoch + 1)

        # test_loss = utils.test_one_epoch(model, val_data_loader,
        #                                       device, epoch, print_freq=50,
        #                                       warmup=True, scaler=scaler)
        # writer.add_scalar('testloss', test_loss, epoch + 1)

        # evaluate on the test dataset
        coco_info, acc, miou = utils.evaluate(model, val_data_loader, device=device)
        miou_map.append(miou)
        acc_map.append(acc)
        writer.add_scalar('mAP', coco_info[1], epoch+1)
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/frcnn/mobile-model-{}.pth".format(epoch))
